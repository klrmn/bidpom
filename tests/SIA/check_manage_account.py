...
class TestManageAccount(SIABaseTest):
    ...
    def test_can_create_new_user_account(self, mozwebqa):
        user = MockUser()
        home = HomePage(mozwebqa.selenium, mozwebqa.timeout, mozwebqa.serverurl)
        ...
        comp_reg = CompleteRegistration(mozwebqa.selenium, mozwebqa.timeout, 
            self.get_confirm_url_from_email(user.primary_email, mozwebqa.timeout), 
            expect='success')

        # verify now logged in
        home_pg = HomePage(mozwebqa.selenium, mozwebqa.timeout, mozwebqa.serverurl)
    ...
    def test_that_user_can_sign_in_and_out(self, mozwebqa):
        ...
        user = self.create_verified_user(mozwebqa.selenium, mozwebqa.timeout, mozwebqa.serverurl)
        home = HomePage(mozwebqa.selenium, mozwebqa.timeout, mozwebqa.serverurl)
